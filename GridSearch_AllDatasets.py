...
l_epsilon = [0.05, 0.1, 0.5]
l_sigma = [0.01, 0.05, 0.1, 0.5, 1, 5, 10, 100]
l_gamma = [1e-6, 1e-4, 1e-2]
...
columns = ['epsilon', 'sigma', 'gamma', 'RMSE', 'NDEI', 'MAE']
result = pd.DataFrame(columns = columns)
for epsilon in l_epsilon:
    for sigma in l_sigma:
        for gamma in l_gamma:
                            
            print(f"epsilon = {epsilon} and sigma = {sigma} and gamma = {gamma}")
        
            # Initializing the model
            model = QKRLS(epsilon = epsilon, sigma = sigma)
            # Train the model
            ...
            NewRow = pd.DataFrame([[epsilon, sigma, gamma, RMSE, NDEI, MAE]], columns = columns)
